...
image:https://github.com/pembana/pembana-rain-gauge/workflows/Java%20CI%20with%20Maven/badge.svg[CI Pembana,link=https://github.com/pembana/pembana-rain-gauge/actions]
image:.github/badges/jacoco.svg[Coverage,link=https://github.com/pembana/pembana-rain-gauge/actions]
image:https://img.shields.io/badge/License-Apache_2.0-blue.svg[License,link=https://opensource.org/licenses/Apache-2.0]

Pembana Rain Gauge is a Java 26 / https://spring.io/projects/spring-boot[Spring Boot] 4.1 application for exploring near-real-time and
...
The initial response is a complete server-rendered https://jte.gg/[JTE] page. Vanilla JavaScript progressively
...
* Station discovery: IEM `HI_DCP` https://mesonet.agron.iastate.edu/geojson/network.py?network=HI_DCP[GeoJSON catalog].
* Native observations: IEM https://mesonet.agron.iastate.edu/cgi-bin/request/hads.py?help=[HADS archive request service].
...
. requests an additional baseline lookback and selects the latest valid reading at or before
   `from`, or starts a partial total at the first in-range reading when no baseline exists;
. sorts readings by valid time and deterministically collapses retransmissions;
. subtracts each previous accumulator from the next one;
. adds nonnegative deltas whose timestamp falls inside the requested period;
. accepts only corroborated near-zero resets or configured rollovers;
. retains but flags unusually large positive increments; and
. reports the last observation actually used as the cutoff.
...
Open http://localhost:8080. The default profile uses the entirely in-memory
...
Pembana Rain Gauge is licensed under the link:LICENSE[Apache License 2.0]. Vendored dependencies retain
